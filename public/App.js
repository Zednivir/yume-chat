document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener("click", function(event) {
        const isHamburgerMenuClicked = event.target.closest('.hamburgerMenu') !== null;
        const isToggleClicked = event.target.closest('#toggle-sidebar') !== null;
        const isHamburgerButtonClicked = event.target.closest('.hamburgerButton') !== null;

        if (!isHamburgerMenuClicked && !isToggleClicked && !isHamburgerButtonClicked) {
            // If the click is outside the hamburger menu, toggle button, and hamburger button
    ...
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            // Update: Use the item method to access the first element
            const fileTabElement = fileTab.item(0);

            if (fileSelected && fileTabElement && fileTabElement.classList.contains('active')) {
            ...
            // Update: Check if fileTabElement is not null before manipulating the class
            if (fileTabElement) {
                fileTabElement.classList.remove('active');
            }
